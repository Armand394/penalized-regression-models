...
rolling_window_forecast_lasso <- function(x,y, lambda_adla){
  ...
  for(i in 1:(n - start_window - h + 1)) {
    ...
    alasso_model <- glmnet(x = x_train, y = y_train, alpha = 1, lambda = lambda_adla)
...
# Brazil
lambda_la_brazil_rgdp <- 0.00225702
lambda_la_brazil_cpi <- 0.002535364

# Costa Rica
lambda_la_cr_rgdp <- 0.001
lambda_la_cr_cpi <- 0.001261857

rolling_window_forecast_lasso(x_brazil_rgdp, log_diff_rgdp_brazil, lambda_la_brazil_rgdp)
rolling_window_forecast_lasso(x_cr_rgdp, log_diff_rgdp_cr, lambda_la_cr_rgdp)
rolling_window_forecast_lasso(x_brazil_cpi, log_diff2_cpi_brazil, lambda_la_brazil_cpi)
rolling_window_forecast_lasso(x_cr_cpi, log_diff2_cpi_cr, lambda_la_cr_cpi)