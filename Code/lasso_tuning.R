...
get_lambda_lasso <- function(x, y){
  ...
  # Calculate prediction accuracy for all lasso lambdas
  for(j in seq_along(lambda_values_adla)) {
    lambda_adla <- lambda_values_adla[j]
    adla_model <- glmnet(x = train_data_x, y = train_data_y, alpha = 1, lambda = lambda_adla)
...
# Brazil
lambda_la_brazil_rgdp <- get_lambda_lasso(x_brazil_rgdp, log_diff_rgdp_brazil)
lambda_la_brazil_cpi <- get_lambda_lasso(x_brazil_cpi, log_diff2_cpi_brazil)

# Costa Rica
lambda_la_cr_rgdp <- get_lambda_lasso(x_cr_rgdp, log_diff_rgdp_cr)
lambda_la_cr_cpi <- get_lambda_lasso(x_cr_cpi, log_diff2_cpi_cr)


print(lambda_la_brazil_rgdp)
print(lambda_la_brazil_cpi)
print(lambda_la_cr_rgdp)
print(lambda_la_cr_cpi)
